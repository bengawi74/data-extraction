...
def make_demo_inputs():
    ...
    (BASE / "data" / "customers.json").write_text(
        json.dumps(
            {
                "customers": [
...
def fetch_json(
    s: requests.Session, url: str, filename: str, log: logging.Logger
) -> pathlib.Path:
    log.info("GET %s", url)
...
def build_dataset(api_base: str, timeout: int, log: logging.Logger) -> pd.DataFrame:
    ...
    users_small = users.loc[:, ["id", "username", "email"]].rename(
    ...
    todos_stats = todos_small.groupby("api_user_id", as_index=False).agg(
        total_todos=("todo_id", "count"), completed_todos=("completed", "sum")
    )
...
def save_outputs(
    df: pd.DataFrame, out_dir: pathlib.Path, write_parquet: bool, log: logging.Logger
):
...
def parse_args():
    ...
    parser.add_argument(
        "--api-base", default=None, help="API base URL (overrides .env)"
    )
    parser.add_argument(
        "--timeout",
        type=int,
    ...
    parser.add_argument(
        "--out-dir", default=str(OUT), help="Output directory (default: data/processed)"
    )
...
def main():
    ...
    api_base = (
        args.api_base or os.getenv("API_BASE", "https://jsonplaceholder.typicode.com")
    ).rstrip("/")
